...
from  LineageType import LineageType
...
def display_lineage(order,txn_ref_no):
    ...
    if (csa_fl==1 and (request_type== LineageType.TOP_TO_BOTTOM_FULL_LINEAGE.name or request_type== LineageType.BOTTOM_TO_TOP_FULL_LINEAGE.name)):
        try:
            ...
            st.plotly_chart(fig) 
            #st.components.v1.html('nx.html')
        except Exception as ex:
            print("Error plotting Graph"+ ex)
            st.markdown("Error Plotting Graph")
    
    st.page_link("pages/app.py", label="New Lookup")
